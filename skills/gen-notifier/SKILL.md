---
name: gen-notifier
description: Send desktop notifications when tasks are complete. This skill should be used when user tells you to notify them when a job is done. By default, assume that all jobs assigned to you will require a notification.
version: 1.0.0
---
...
```bash
terminal-notifier -title "{DESCRIPTION OF JOB}" -message "{STATUS_OF_JOB}"
```
...
```bash
terminal-notifier -title "API Integration Implementation" -message "completed"
```

### Needs User Input
```bash
terminal-notifier -title "Database Migration Setup" -message "needs_input"
```

### Encountered Errors
```bash
terminal-notifier -title "Build and Test Suite" -message "errors"
```
...
4. Sends notification: terminal-notifier -title "Authentication Feature" -message "completed"
...
4. Sends notification: terminal-notifier -title "Database Migration Setup" -message "needs_input"
...
4. Sends notification: terminal-notifier -title "Test Suite Execution" -message "errors"
...
```
- Notification: "Authentication Feature" -message "completed"
```
